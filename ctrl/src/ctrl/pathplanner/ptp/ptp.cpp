...
Trajectory* Ptp::get_ptp_trajectory(Configuration* _start_cfg, Configuration* _end_cfg, bool sync)
{
    ...
    if (PTP_DEBUG) {
        printf("------------------------------------------------------------------------\n");
        printf("Now calculating %s PTP movement:\n", sync ? "synchronous" : "asynchronous");
    ...
    double full_duration = 0; // maximum of time_final -> duration of the full movement
    for (int i = 0; i < 6; i++) {
    ...
    if (PTP_DEBUG) {
        for (int i = 0; i < 6; i++) {
            if (time_c[i] * 2 >= time_final[i]) printf("Axis %d will take %f seconds using a maximum velocity profile\n", i, time_maxvelocity[i]);
            else printf("Axis %d will take %f seconds using a trapezoidal profile (%f seconds for acceleration/deceleration)\n", i, time_final[i], time_c[i]);
        }
        printf("START: %f %f %f %f %f %f\n", start_cfg[0], start_cfg[1], start_cfg[2], start_cfg[3], start_cfg[4], start_cfg[5]);
    }
    ...
    for (double t = 0; t < full_duration; t += SECONDS_PER_STEP) {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            if (time_c[i] * 2 >= time_final[i]) {
                ...
                if (sync) ti = t * (time_maxvelocity[i] / full_duration);
                if (ti < time_maxvelocity[i] / 2.0) {
                    if (PTP_DEBUG) printf("       Axis %d: max-velocity, accelerating\n", i+1);
                    new_cfg[i] = start_cfg[i] + 2.0 * pow(ti / time_maxvelocity[i], 2.0) * distance[i];
                } else if (ti < time_maxvelocity[i]) {
                    if (PTP_DEBUG) printf("       Axis %d: max-velocity, decelerating\n", i+1);
                    new_cfg[i] = start_cfg[i] +
                                 (-1.0 + 4.0 * (ti / time_maxvelocity[i]) - 2.0 * pow(ti / time_maxvelocity[i], 2.0)) * distance[i];
                } else {
                    new_cfg[i] = end_cfg[i];
                }
            } else {
                // we need a trapezoidal profile
                if (sync) ti = t * (time_final[i] / full_duration);
                int sign = start_cfg[i] < end_cfg[i] ? 1 : -1;
                if (ti < time_c[i]) {
                    if (PTP_DEBUG) printf("       Axis %d: trapezoidal, accelerating\n", i+1);
                    new_cfg[i] = start_cfg[i] + sign * 0.5 * (MAX_ACCELERATION[i] * TO_RAD) * pow(ti, 2);
                } else if (ti <= time_final[i] - time_c[i]) {
                    if (PTP_DEBUG) printf("       Axis %d: trapezoidal, reached maximum velocity\n", i+1);
                    new_cfg[i] =
                            start_cfg[i] + sign * (MAX_ACCELERATION[i] * TO_RAD) * time_c[i] * (ti - (time_c[i] / 2));
                } else if (ti <= time_final[i]) {
                    if (PTP_DEBUG) printf("       Axis %d: trapezoidal, decelerating\n", i+1);
                    new_cfg[i] = end_cfg[i] - sign * 0.5 * (MAX_ACCELERATION[i] * TO_RAD) * pow(time_final[i] - ti, 2);
                } else {
                    new_cfg[i] = end_cfg[i];
                }
            }
