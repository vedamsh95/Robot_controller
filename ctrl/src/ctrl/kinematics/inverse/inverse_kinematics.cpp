...
vector<double> vec_phi4, vec_phi5, vec_phi6;
...
vector<Configuration *> *InvKinematics::get_inv_kinematics(SixDPos *_pos) {
    ...
    //elbow singularity
    double X = sqrt(pow(xc, 2) + pow(yc, 2)) - m;
    double Y = zc - n;
    double XY_square = sqrt(pow(X, 2) + pow(Y, 2));
    double d2 = sqrt((o * o) + (b * b));
    if (XY_square >= pow((a + d2), 2) - margin_point) {
        xc = sqrt(pow((X * (1 + s) + m), 2) - pow(yc, 2));
        zc = Y * (1 + s) + n;
    }

    double phi1_standard = atan2(yc, -(xc)) * 180 / M_PI;
    cout << "phi standard " << phi1_standard << endl;
    ...
    if (xc < 0 && yc > 0) {
        ...
        standardsol_1 = specialCase1(xc, yc, zc, m, d1);
    } else if (xc == 0 && yc < 0) {
        standardsol_1 = specialCase2(xc, yc, zc, m, d1);
    }

    // Overhead Singularity/shoulder singularity
    if ((xc <= 0 + margin_point && xc >= 0 - margin_point) && (yc <= 0 + margin_point && yc >= margin_point)) {
        phi1 = 0;
        cout << " phi1 with overhead singularity " << phi1 << endl;
        standardsol_1 = standardCase(phi1, d1, m, xc, yc, zc);
    ...
    // All the possible configurations are collected.
...
//calculating forward angles for phi2 and phi3
...
//calculating backward angles for phi2 and phi3
...
//checking the limits for phi2 and phi3
...
vector<vector<double>> *InvKinematics::specialCase1(double xc, double yc, double zc, double m, double d1) {
    ...
    if (yc > m) {
        ...
        results->push_back(special_1_1);
        results->push_back(special_1_2);
        return results;
    }
...
vector<vector<double>> *InvKinematics::specialCase2(double xc, double yc, double zc, double m, double d1) {
...
//transformation matrices for joints 1,2,3
...
vector<vector<double>> InvKinematics::phi4_5_6(TMatrix *R36) {
...
//double phi5_2 = atan2( (-1)*sqrt(1-(R36->get(3,3)*R36->get(3,3))),(-1)*R36->get(3,3))* 180 /M_PI;
    ...
    set1.clear();
    set2.clear();
    set3.clear(), set4.clear(), set5.clear(), set6.clear(), set7.clear(), set8.clear(), vec_456.clear();
